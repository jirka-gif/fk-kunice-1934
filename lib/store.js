...
import { DEFAULTS, mergeStored, clone, INBOX_KEYS, inboxOtisk, emptyNextMatch, emptyMatchDetail, emptyLastMatch, emptyCamp, emptyNews, slugify, emptyOpponent, opponentKey, collectOpponentNames, cleanClubName, emptySponsor, emptyGalleryItem, emptyReservation, emptyRegistration } from '@/lib/defaults';
...
let _apply = null;
let _saveTimer = null;
// O čem víme v příchozích seznamech (poptávky, zprávy, přihlášky, návrhy zápasů).
// Roste o všechno, co jsme viděli i sami založili. Při ukládání z toho spočítáme,
// co zmizelo = co uživatel smazal, a pošleme to serveru. Ten pak pozná rozdíl mezi
// „tohle jsem smazal" a „o tomhle jsem nevěděl" a poptávku doručenou mezitím
// nepřepíše. Viz `mergeInbox` v lib/defaults.js.
let _inboxZname = null;

function zapamatujInbox(content) {
  if (!_inboxZname) _inboxZname = {};
  for (const key of INBOX_KEYS) {
    const mnozina = _inboxZname[key] || (_inboxZname[key] = new Set());
    for (const it of Array.isArray(content[key]) ? content[key] : []) mnozina.add(inboxOtisk(it));
  }
}

// Otisky, které jsme znali a v posílaném obsahu už nejsou → uživatel je smazal.
function smazaneOtisky(content) {
  if (!_inboxZname) return null;
  const out = {};
  for (const key of INBOX_KEYS) {
    const ted = new Set((Array.isArray(content[key]) ? content[key] : []).map(inboxOtisk));
    const pryc = [...(_inboxZname[key] || [])].filter((o) => !ted.has(o));
    if (pryc.length) out[key] = pryc;
  }
  return out;
}
...
function scheduleServerSave() {
  ...
  _saveTimer = setTimeout(async () => {
    ...
    const smazane = smazaneOtisky(_data);
    try {
...
function commit(next) {
  _data = next;
  // I to, co v administraci právě vzniklo, patří mezi „vím o tom" — jinak by to
  // po smazání ve stejném sezení server vracel zpátky jako neznámou položku.
  zapamatujInbox(next);
  writeCache(next);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        _data = merged;
        // Až ze serveru, ne z mezipaměti — musíme vědět o tom, co je opravdu uložené.
        zapamatujInbox(merged);
        writeCache(merged);
        setData(merged);
      } catch {
