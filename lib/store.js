...
// zápasová data týmu — Muži A nese stávající reálná data, ostatní prázdné
function teamMatch(id) {
  if (id === 'muziA') return { nextMatch: club.nextMatch, results: club.results, table: club.leagueTable, matchDetail: club.matchDetail };
  return { nextMatch: emptyNextMatch(), results: [], table: [], matchDetail: emptyMatchDetail() };
}

function buildDefaults() {
  return {
    club: club.club,
    teams: club.teams.map((t) => ({ ...t, ...teamMatch(t.id) })),
...
function readLS() {
  try {
    ...
    return normalizeTeams({ ...clone(DEFAULTS), ...saved }, saved);
  } catch { return null; }
}
