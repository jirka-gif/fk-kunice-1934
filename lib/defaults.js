...
export function mergeStored(saved) {
  ...
  const merged = normalizeGallery(normalizeSponsors(normalizeOpponents(normalizeSocial(normalizeProposals(normalizeNews(normalizeCamps(normalizeTeams({ ...clone(DEFAULTS), ...saved }, saved), saved)))))));
