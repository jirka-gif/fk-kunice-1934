...
import { emptyRentalSettings, normalizeRentalSettings, slotEnd, czechDate, REPEAT_MODES } from '@/lib/rental';

export const clone = (o) => JSON.parse(JSON.stringify(o));

// prázdné zápasové struktury pro tým bez dat
export function emptyNextMatch() {
  // `when` je text, který se ukazuje na webu („NE 16:30 · III. TŘÍDA").
  // Nezadává se ručně — skládá se z `dateISO` a `competition` přes
  // `matchWhenText()`, aby se datum nevyplňovalo dvakrát.
  return { home: { short: 'FK', name: 'KUNICE', side: 'Domácí' }, away: { short: '?', name: 'SOUPEŘ', side: 'Hosté' }, when: '', competition: '', venue: 'Areál Kunice', dateISO: '' };
}

// Odehrané zápasy patří od nejnovějšího. Záznamy bez data si drží pořadí,
// v jakém jsou — u nich se ručně přesouvá šipkami.
export function sortResults(results) {
  const list = Array.isArray(results) ? results.slice() : [];
  return list
    .map((r, i) => ({ r, i }))
    .sort((a, b) => {
      const da = a.r && a.r.dateISO ? a.r.dateISO : '';
      const db = b.r && b.r.dateISO ? b.r.dateISO : '';
...
// Z data a názvu soutěže složí text pro web. Prázdné datum i prázdná soutěž
// se prostě vynechají, takže z neúplného zadání nevznikne „ · " bez obsahu.
export function matchWhenText(dateISO, competition) {
  const comp = String(competition || '').trim();
  const d = dateISO ? new Date(dateISO) : null;
  if (!d || isNaN(d)) return comp;
  const cas = `${DNY_ZKRATKA[d.getDay()]} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  return comp ? `${cas} · ${comp}` : cas;
}
...
export function emptyNews() {
  // `draft` = rozepsaná novinka. Na webu se nezobrazí, dokud ji člověk
  // nezveřejní — nová novinka tak nesvítí veřejnosti prázdná.
  return { id: '', category: 'Klub', title: '', text: '', body: '', date: '', image: '', draft: false };
}
...
export function emptyReservation() {
  return {
    ...
    // E-mail a telefon zvlášť. Dřív bylo jen `contact` jako volný text — z toho
    // se nedalo odepsat, protože nešlo poznat, co v poli je.
    email: '', phone: '',
    dateISO: '', from: '', to: '',
    date: '', time: '',
    // Dlouhodobý pronájem: `repeat` říká jak často, `repeatUntil` dokdy,
    // `skipDates` vyjímá konkrétní dny (prázdniny, turnaj).
    repeat: '', repeatUntil: '', skipDates: [],
    // Přání z webu: „chtěl bych ten termín pravidelně". Je to jen poznámka
    // k poptávce — `occursOn` na ně nesahá, takže se zabere jen první termín.
    // Skutečnou sérii založí až klub tím, že vyplní `repeat` (tlačítko v adminu).
    // Kdyby to opakování zabíralo hned, jedna nepotvrzená poptávka by zablokovala
    // půl roku kalendáře.
    repeatWanted: '', repeatUntilWanted: '',
    note: '', source: 'web', status: 'nová', createdAt: '',
    // Co klubu odešlo žadateli — datum, předmět, text. Aby bylo dohledatelné,
    // co jsme komu slíbili.
    messages: [],
  };
}

// Ze starého pole `contact` („602 123 456 · jan@novak.cz") vytáhne e-mail.
function emailFromContact(contact) {
  const match = String(contact || '').match(/[^\s·,;]+@[^\s·,;]+\.[a-zA-Z]{2,}/);
  ...
  data.reservations = (Array.isArray(data.reservations) ? data.reservations : [])
    .map((r, i) => {
      ...
      return {
        ...
        // Starší záznamy mají jen `contact` — když v něm je zavináč, vezmeme ho
        // jako e-mail, ať jde odepsat i na to, co dorazilo dřív.
        email: String(src.email || emailFromContact(src.contact) || ''),
        phone: String(src.phone || ''),
        repeat: REPEAT_MODES.includes(src.repeat) ? src.repeat : '',
        repeatUntil: /^\d{4}-\d{2}-\d{2}$/.test(src.repeatUntil) ? src.repeatUntil : '',
        repeatWanted: REPEAT_MODES.includes(src.repeatWanted) ? src.repeatWanted : '',
        repeatUntilWanted: /^\d{4}-\d{2}-\d{2}$/.test(src.repeatUntilWanted) ? src.repeatUntilWanted : '',
        skipDates: Array.isArray(src.skipDates) ? src.skipDates.filter((d) => /^\d{4}-\d{2}-\d{2}$/.test(d)) : [],
        messages: Array.isArray(src.messages) ? src.messages.slice(0, 50) : [],
      };
    })
    .slice(0, 500);
  return data;
...
// --- PŘIHLÁŠKY DO KLUBU ----------------------------------------------------
// Přihláška z náborového formuláře na webu. Stejný tok jako u zpráv a rezervací:
// přijde jako `nová`, klub ji vyřídí nebo zamítne. Starý ukázkový zápis
// (iniciály, barva, štítek) se převede na skutečná pole.
export function emptyRegistration() {
  return {
    id: '', name: '', birthdate: '', team: '',
    parent: '', contact: '', note: '',
    // E-mail zvlášť, aby šlo rodiči odepsat. `contact` je volný text,
    // ze kterého se adresa spolehlivě vytáhnout nedá.
    email: '', phone: '',
    // Co komu odešlo — datum, předmět, text, výsledek.
    messages: [],
    status: 'nová', source: 'web', createdAt: '',
  };
}

export function normalizeRegistrations(data) {
  data.cmsRegistrations = (Array.isArray(data.cmsRegistrations) ? data.cmsRegistrations : [])
    .map((r, i) => {
      const src = r && typeof r === 'object' ? r : {};
      // převod starých štítků: new → nová, ok → vyřízená, wait → nová
      const legacy = { new: 'nová', ok: 'vyřízená', wait: 'nová' }[src.tg];
      const status = ['nová', 'vyřízená', 'zamítnutá'].includes(src.status) ? src.status : (legacy || 'nová');
      const out = {
        ...emptyRegistration(), ...src, id: src.id || `prihlaska-${i + 1}`, status,
        // u starších přihlášek vytáhneme e-mail z volného kontaktu
        email: String(src.email || emailFromContact(src.contact) || ''),
        messages: Array.isArray(src.messages) ? src.messages.slice(0, 50) : [],
      };
      // vizuální zbytky z původního návrhu už nepotřebujeme
      delete out.ini; delete out.bg; delete out.tag; delete out.tg;
      return out;
    })
    .slice(0, 500);
  return data;
...
// --- PLOCHY K PRONÁJMU -----------------------------------------------------
// Dřív existovaly dva seznamy hřišť: `facilities` (blok na homepage) a
// `rentalPlans` (ceník + volba plochy v rezervaci). Vyplňovaly se dvakrát
// a chodily si rozcházet. Teď je seznam jeden — homepage bere první tři.
export function normalizeRentalPlans(data) {
  const plans = Array.isArray(data.rentalPlans) ? data.rentalPlans : [];
  const legacy = Array.isArray(data.facilities) ? data.facilities : [];
  // hřiště, které bylo jen na homepage, o sebe nepřijde
  const chybejici = legacy.filter((f) => f && f.name && !plans.some((p) => p && p.name === f.name));
  // obsazenost se počítá z rezervací, ručně psané „obsazené dny" jsou přežitek
  delete data.rentalBusyDays;
  data.rentalPlans = [...plans, ...chybejici].map((p, i) => {
    const src = p && typeof p === 'object' ? p : {};
    return {
      name: String(src.name || ''),
      spec: String(src.spec || ''),
      price: String(src.price || ''),
      status: src.status === 'OBSAZENO' ? 'OBSAZENO' : 'VOLNO',
      img: String(src.img || ''),
...
export function buildDefaults() {
  return {
    ...
    people: club.people,
    cmsRegistrations: normalizeRegistrations({ cmsRegistrations: clone(club.cmsRegistrations) }).cmsRegistrations,
...
// U těchto sekcí se chybějící vnořené klíče doplní z výchozích hodnot,
// aby starý uložený obsah nepřišel o nově přidané texty.
const DEEP_SECTIONS = ['homeTexts', 'pageTexts', 'footer', 'club'];
...
export function mergeStored(saved) {
  ...
  const merged = normalizeRentalPlans(normalizeRegistrations(normalizeReservations(normalizeGallery(normalizeSponsors(normalizeOpponents(normalizeSocial(normalizeProposals(normalizeNews(normalizeCamps(normalizeTeams({ ...clone(DEFAULTS), ...saved }, saved), saved))))))))));
...
// =============================================================================
//  PŘÍCHOZÍ POŠTA — SLOUČENÍ MÍSTO PŘEPSÁNÍ
//
//  Administrace posílá při každém uložení CELÝ obsah tak, jak si ho načetla při
//  otevření stránky. Když mezitím dorazí poptávka, zpráva nebo přihláška, uložení
//  by ji přepsalo starší verzí seznamu a položka by zmizela. Do těchhle seznamů
//  totiž nepřispívá jen administrace — píše do nich veřejný formulář (/api/submit)
//  a stahovač zápasů (/api/matches).
//
//  Pravidlo: položku, která je v uloženém obsahu a v příchozím chybí, vrátíme
//  zpátky — POKUD administrace neřekla, že ji smazala. Seznam smazaných chodí
//  v hlavičce `x-fk-inbox-removed`.
//
//  Dřív se místo toho porovnávalo, co je novější. Vyrobilo to dvě různé chyby
//  (český zápis data se v textovém porovnání choval obráceně; položku, kterou
//  administrace sama založila, pak nešlo smazat), takže se to vyhodilo. Seznam
//  smazaných je přesný a nezávisí na tvaru dat.
// =============================================================================

// Klíče obsahu, do kterých přispívá i někdo jiný než administrace.
export const INBOX_KEYS = ['reservations', 'messages', 'cmsRegistrations', 'matchProposals'];

// Totožnost položky. Zprávy z kontaktu `id` nemají, proto náhradní otisk
// z data odeslání, adresy a začátku textu.
export function inboxOtisk(item) {
  const it = item && typeof item === 'object' ? item : {};
  if (it.id) return `id:${it.id}`;
  return `fp:${it.date || ''}|${it.email || ''}|${String(it.text || '').slice(0, 60)}`;
}

// Otisky všech položek v příchozích seznamech — administrace si podle nich drží
// přehled, o čem ví, a při ukládání pošle rozdíl (co zmizelo = co smazala).
export function inboxIds(content) {
  const out = {};
  for (const key of INBOX_KEYS) {
    out[key] = (Array.isArray(content && content[key]) ? content[key] : []).map(inboxOtisk);
  }
  return out;
}

// Sloučí jeden seznam. `ulozene` = co je v databázi, `prichozi` = co poslal klient,
// `smazane` = otisky, které klient záměrně odstranil.
function sloucSeznam(ulozene, prichozi, smazane) {
  if (!Array.isArray(prichozi)) return ulozene; // klient klíč neposlal → neměníme
  if (!Array.isArray(ulozene) || ulozene.length === 0) return prichozi;

  const poslane = new Set(prichozi.map(inboxOtisk));
  const smazaneSet = new Set(Array.isArray(smazane) ? smazane : []);

  const vracene = ulozene.filter((it) => {
    const o = inboxOtisk(it);
    if (poslane.has(o)) return false;      // klient ji poslal, řeší si ji sám
    if (smazaneSet.has(o)) return false;   // klient ji smazal, respektujeme to
    return true;                           // o téhle klient nevěděl → dorazila mezitím
  });

  if (vracene.length === 0) return prichozi;
  // Vrácené položky dorazily později, a seznamy jsou řazené od nejnovějšího.
  return [...vracene, ...prichozi].slice(0, 500);
}

// Vezme obsah poslaný administrací a doplní do něj poštu, která mezitím dorazila.
// `smazane` = { klíč: [otisk, …] }, tedy co administrace záměrně odstranila.
// Bez něj se nic nemaže — raději nechat navíc než nenávratně ztratit.
export function mergeInbox(ulozeny, prichozi, smazane) {
  if (!prichozi || typeof prichozi !== 'object') return prichozi;
  if (!ulozeny || typeof ulozeny !== 'object') return prichozi;
  const s = smazane && typeof smazane === 'object' ? smazane : {};
  const out = { ...prichozi };
  for (const key of INBOX_KEYS) out[key] = sloucSeznam(ulozeny[key], prichozi[key], s[key]);
  return out;
}
