...
function teamMatch(id) {
  ...
  if (id === 'muziA' && m) return { ...m, sourceUrl: m.sourceUrl || m.facrUrl || '', results: club.results, matchDetail: club.matchDetail };
  if (m) return { ...m, sourceUrl: m.sourceUrl || m.facrUrl || '', results: [], matchDetail: emptyMatchDetail() };
  return { nextMatch: emptyNextMatch(), lastMatch: emptyLastMatch(), results: [], table: [], facrUrl: '', sourceUrl: '', matchDetail: emptyMatchDetail() };
}
...
const CLUB_PREFIXES = ['fk', 'tj', 'sk', 'fc', 'af', 'sokol', 'af'];
...
export function mergeStored(saved) {
  ...
  const merged = normalizeOpponents(normalizeSocial(normalizeProposals(normalizeNews(normalizeCamps(normalizeTeams({ ...clone(DEFAULTS), ...saved }, saved), saved)))));
