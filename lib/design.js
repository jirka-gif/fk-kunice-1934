...
// Pozadí dlaždice: buď nahraná fotka, nebo pojmenovaný barevný přechod.
// Vrací hotovou hodnotu do CSS `background`, proto se fotka balí do url().
export function photo(value) {
  const v = String(value || '');
  if (PH[v]) return PH[v];
  if (!v) return PH.slate;
  // nahraná fotka (data URL) nebo odkaz na obrázek
  if (v.startsWith('data:') || v.startsWith('http') || v.startsWith('/')) return `url(${v})`;
  return v; // vlastní CSS (gradient apod.)
}
...
export function wldBadge(t) {
  ...
  return `width:24px;height:24px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12px;font-family:Inter;${map[t] || map.V}`;
}
