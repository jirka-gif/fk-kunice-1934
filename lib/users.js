...
//  závislosti a funguje na Vercelu i lokálně). Uživatelé jsou v samostatném
//  úložišti (site_auth), aby se nikdy nedostali do veřejného obsahu webu.
// =============================================================================
import { getStoredAuth, saveStoredAuth } from '@/lib/db';
import { defaultRoles, normalizePermissions, permissionsForRole } from '@/lib/permissions';
...
export function normalizeAuth(stored) {
  ...
  if (!roles.some((r) => r.id === 'spravce')) roles.unshift(defaultRoles()[0]);
