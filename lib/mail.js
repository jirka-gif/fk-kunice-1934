...
export function registrationMail(registration) {
  ...
  const lines = [
    ...
    '',
    `Jméno:     ${g.name || '—'}`,
    g.birthdate ? `Narozen(a): ${g.birthdate}` : '',
    `Kategorie: ${g.team || '—'}`,
    `Rodič:     ${g.parent || '—'}`,
    `Kontakt:   ${g.contact || g.email || '—'}`,
    g.note ? `Poznámka: ${g.note}` : '',
    '',
...
// Kam posílat upozornění klubu. Adresa z nastavení pronájmu, a když není
// vyplněná, spadne se na klubový e-mail z Nastavení — jinak by upozornění
// tiše nikam nechodila a nikdo by se to nedozvěděl.
export function notifyAddress(content) {
  const c = content || {};
  return String(c.rentalSettings?.notifyEmail || c.club?.email || '').trim();
}
