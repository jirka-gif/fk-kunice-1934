...
export function reservationMail(reservation) {
  ...
  const prani = OPAKOVANI_SLOVY[r.repeatWanted]
    ? `Žádá o pravidelný termín: ${OPAKOVANI_SLOVY[r.repeatWanted]}${r.repeatUntilWanted ? `, přibližně do ${r.repeatUntilWanted}` : ''}.`
    : '';
  const lines = [
    ...
    prani ? 'Opakování se samo nezabírá — zapnout ho jde v detailu poptávky.' : '',
  ].filter((l) => l !== '');
  return {
