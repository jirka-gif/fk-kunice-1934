...
export default function Footer() {
  const { club, footer } = useContent();
  // ikona sociální sítě se zobrazí jen tehdy, když má klub vyplněný odkaz
  const socials = [
    { key: 'instagram', href: footer.social.instagram, icon: <Ig /> },
    { key: 'facebook', href: footer.social.facebook, icon: <Fb /> },
    { key: 'twitter', href: footer.social.twitter, icon: <Tw /> },
  ].filter((s) => s.href);
  return (
    <footer style={{ background: '#0c0c0e', color: '#fff', padding: '84px 0 34px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px', position: 'relative' }}>
          ...
          <div>
            ...
            {socials.length > 0 && (
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                {socials.map((s) => (
                  <Hov key={s.key} as="a" href={s.href} target="_blank" rel="noopener noreferrer" style="width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,.07);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background .25s" hover="background:#C1121F">{s.icon}</Hov>
                ))}
              </div>
            )}
          </div>

          {[footer.columnA, footer.columnB].map((col, ci) => (
            <div key={ci}>
              <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '2px', color: 'rgba(255,255,255,.42)', marginBottom: 18 }}>{col.title}</div>
              {(col.links || []).map((l, i) => (
                <Hov key={i} as={Link} href={l.href || '/'} style="color:rgba(255,255,255,.7);font-size:14px;font-weight:500;margin-bottom:13px;cursor:pointer;transition:color .2s;display:block" hover="color:#fff">{l.label}</Hov>
              ))}
            </div>
          ))}
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '2px', color: 'rgba(255,255,255,.42)', marginBottom: 18 }}>{footer.contactTitle}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 28, flexWrap: 'wrap', gap: 12 }}>
          <span style={{ color: 'rgba(255,255,255,.4)', fontSize: 13, fontWeight: 500 }}>{footer.copyright}</span>
          <span style={{ color: 'rgba(255,255,255,.4)', fontSize: 13, fontWeight: 600, fontFamily: "'Bebas Neue'", letterSpacing: '1px' }}>{footer.claim}</span>
        </div>
      </div>
    </footer>
  );
