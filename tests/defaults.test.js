...
describe('toPlayer', () => {
  ...
  it('doplní chybějící pole u objektu a zachová vyplněná', () => {
    const p = toPlayer({ name: 'Petr Svoboda', birthdate: '2004-03-12' }, 4);
    expect(p.name).toBe('Petr Svoboda');
    expect(p.birthdate).toBe('2004-03-12');
    expect(p.number).toBe(5);
    expect(p.position).toBeTruthy();
  });

  it('drží jen údaje, které klub opravdu vyplňuje (věk se počítá z data narození)', () => {
    expect(Object.keys(toPlayer('Jan Novák', 0)).sort()).toEqual(['birthdate', 'name', 'number', 'photo', 'position']);
  });
});
